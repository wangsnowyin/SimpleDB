...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            //oincost(t1 join t2) = scancost(t1) + ntups(t1) x scancost(t2) IO cost
            //                      + ntups(t1) x ntups(t2)  CPU cost
            double totalcost = card1 * card2 + cost1 + card1 * cost2;
    ...
    public Vector<LogicalJoinNode> orderJoins(
       ...
       */
        ...
        Set<LogicalJoinNode> finalset = new HashSet<LogicalJoinNode>();
        finalset.addAll(joins);
        for(int i = 1; i <= joins.size(); i++){
            Set<Set<LogicalJoinNode>> set = enumerateSubsets(joins, i);
            for(Set<LogicalJoinNode> curSet : set){
                CostCard bestPlan = new CostCard();
                bestPlan.cost = Double.MAX_VALUE;
                for(LogicalJoinNode node : joins){
                    CostCard curPlan = computeCostAndCardOfSubplan(stats, filterSelectivities, node, curSet, bestPlan.cost, optjoin);
                    if(curPlan != null && curPlan.cost < bestPlan.cost){
                        bestPlan = curPlan;
                    }
                }
                optjoin.addPlan(curSet, bestPlan.cost, bestPlan.card, bestPlan.plan);
            }
        } 
        if(explain){
            printJoins(joins, optjoin, stats, filterSelectivities);
        }
        return optjoin.getOrder(finalset);
    }
